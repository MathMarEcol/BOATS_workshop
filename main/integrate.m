    ...
    for indv=1:outmode.(outmode.modes{indm}).nvar
      % Defines output size according to variable type
       ...
       case '4D'
          ...
          % Saves the variable after integral over size dimensions
             outmode.(outmode.modes{indm}).(outmode.(outmode.modes{indm}).var_outn{indv}) = ...
                                           zeros(ontime,FORC.nlat,FORC.nlon,ECOL.nfishmip);
          case '2di_si'
       ...
       case '3D'
          ...
          % Saves the entire variable without any processing
             outmode.(outmode.modes{indm}).(outmode.(outmode.modes{indm}).var_outn{indv}) = ...
                                           zeros(ontime,FORC.nlat,FORC.nlon); 
          case '2di'
    ...
    if ~isempty(iit);
       ...
       for indv=1:outmode.(outmode.modes{indm}).nvar
          ...
          case '4D'
             ...
             case 'fishmip' % ADDED BY RYAN HENEGHAN (JULY 2019) SEE DESCRIPTION IN 'initialize_output.m'
             % Saves the variable after integral over 
                tmpvar1 = repmat(squeeze(nansum( tmpvar .* STRU.delfm_4d,3)) * CONV.mmolC_2gC, [1 1 1 ECOL.nfishmip]);
                tmpvar2 = squeeze(nansum(tmpvar1 .* STRU.fishmip_4d,3))+ STRU.mask_land_g_nan_fishmip;
                outmode.(outmode.modes{indm}).(tvar_outn{indv})(iit,:,:,:)  = ...
                squeeze(outmode.(outmode.modes{indm}).(tvar_outn{indv})(iit,:,:,:))  + ...
          ...
          case '3D'
             ...
             case 'fishmip_tsb'
             % FISHMIP OUTPUT: ADDED BY RYAN HENEGHAN (JULY 2019) In each lat,lon grid square total system
             % carbon biomass. SINCE PHYTOPLANKTON IS PRIMARY PRODUCTION,
             % NOT BIOMASS, THIS HAS BEEN LEFT INCOMPLETE.
        %     tmpvar1 = 
        %        outmode.(outmode.modes{indm}).(tvar_outn{indv})(iit,:,:)  = ...
        %        squeeze(outmode.(outmode.modes{indm}).(tvar_outn{indv})(iit,:,:))  + ...
        %                                      tmpvar/outmode.(outmode.modes{indm}).ndt(iit) + ...
