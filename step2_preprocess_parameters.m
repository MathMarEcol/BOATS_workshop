...
addpath(join([root_dir, 'BOATS_workshop/BOATS_master/preprocess'])); 
...
% Import Forcing Paths *******************************
bathy = join([projectdir, 'processed_forcings/bathy.mat']);
longs = join([projectdir, 'processed_forcings/cesm_lons.mat']);
lats = join([projectdir, 'processed_forcings/cesm_lats.mat']);
masker = join([projectdir, 'processed_forcings/cesm_mask.mat']);
surf_area = join([projectdir, 'processed_forcings/cesm_area.mat']);
intpp_pi = join([projectdir, 'processed_forcings/cesm_clim_intpp.mat']);
temp_pi = join([projectdir, 'processed_forcings/cesm_clim_temp.mat']); 
...
ntime = 3012;                                       % ntime = t distinct time steps per forcing
ngroup = 3;                                         % ngroup = g fish groups per forcing (facultativ)
nensemble = 5;                                      % nensemble = e ensembles per forcing (facultativ)

% mask
mask_path = masker; % Path of forcing dataset where is the mask
mask_var = 'mask';          % Name of mask variable in mask_path
wet=0;                                              % Value for ocean cells in mask_var
dry=1;                                            % Value for continent cells in mask_var
mask_dim = [nlon nlat 1 1 1];                       % Dimension of the mask forcing generated

% coordinates lon/lat
lon_path = longs;  % Path of forcing dataset where is the longitude
lon_var = 'lons';               % Name of longitude variable in lon_var
lat_path = lats;  % Path of forcing dataset where is the latitude
lat_var = 'lats';               % Name of latitude variable in lat_var
lon_dim = [nlon nlat 1 1 1];                        % Dimension of the longitude array generated
lat_dim = [nlon nlat 1 1 1];                        % Dimension of the latitude array generated

% surface
surf_path = surf_area;    % Path of forcing dataset where is the surface
surf_var = 'tarea';                   % Name of surface variable in surf_path
surf_unit = '[m^2]';                               % surf_var unit ([km^2] or [m^2] or ??)
surf_dim = [nlon nlat 1 1 1];                       % Dimension of the surface array generated

% Ecological forcing paths and characteristics *****
% depth
depth_path = bathy; % Path of forcing dataset where is the depth
depth_var = 'bathy';            % Name of depth variable in depth_path
...
depth_dim = [nlon nlat 1 1 1];                      % Dimension of the depth array generated

% npp
npp_path = intpp_pi;  % Path of forcing dataset where is the primary production
npp_var = 'intpp_climate';                % Name of primary porduction variable in npp_path
npp_unit = '[mmolC m^-2 s^-1]';                       % npp_var unit ([mgC m^-2 d^-1] or ??)
npp_dim = [nlon nlat ntime 1 1];                    % Dimension of the npp array generated

% temperature
temp_path = temp_pi; % Path of forcing dataset where is the temperature
temp_var = 'temp_climate';           % Name of temperature variable in temp_path         
temp_unit = '[degC]';                               % temp_var unit ([degC])
temp_dim = [nlon nlat ntime 1 1];                   % Dimension of the temp array generated
...
if create_ecology
    ...
    save(join([root_dir, 'BOATS_workshop/BOATS_master/Ecological.mat']),'Ecological','-v7.3')
end
